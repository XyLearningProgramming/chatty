...
| `CHATTY_ENV` | Contents of `chatty.env` secret file (see `deploy/secret/chatty.env.example`) | LLM endpoint, API key, Postgres URI, Redis URI |

**`CHATTY_ENV` contents** should include at minimum (see `deploy/secret/chatty.env.example` for full reference):
